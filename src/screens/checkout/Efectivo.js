...
const Efectivo = () => {
  ...
  const [referenciaEfectivoFocused, setReferenciaEfectivoFocused] = useState(false);
  ...
  const { clienteorden, restauranteorden } = useContext(FirebaseContext);
  ...
  const sendingDates = async (referenciaEfectivo, image) => {
    // Validaciones
    if (!referenciaEfectivo) {
      Alert.alert('La cantidad no puede estar vacía', '', [
        {
          text: 'Ok',
        },
      ]);
    ...
    try {
      ...
      const efectivoObj = {
        ...
        cantidadefectivo: referenciaEfectivo,
  ...
  const referenciaEfectivoFocus = () => {
    setReferenciaEfectivoFocused(true);
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      <KeyboardAwareScrollView enableOnAndroid>
        <View style={{ alignItems: 'center' }}>
          <Text style={{ fontWeight: 'bold', color: 'red' }}>Captura del Efectivo</Text>
          <Text style={{ fontWeight: 'bold', color: 'red' }}>Total a pagar: $ {pagototal}</Text>
        </View>
        <View style={styles.editForm}>
          <Subtitle2 style={styles.overline}>Escriba la cantidad del efectivo:</Subtitle2>
          <UnderlineTextInput
            // ref={(ref) => {
            //   emailRef = ref;
            // }}

            onChangeText={(referenciaEfectivo) => setReferenciaEfectivo(referenciaEfectivo)}
            onFocus={referenciaEfectivoFocus}
            inputFocused={referenciaEfectivoFocused}
          ...
          <View style={{ paddingHorizontal: 60, paddingVertical: 15 }}>
            <Button
              onPress={() => {
                sendingDates(referenciaEfectivo, image);
              }}

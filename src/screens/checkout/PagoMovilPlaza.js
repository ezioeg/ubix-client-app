...
const PagoMovilPlaza = () => {
  const [image, setImage] = useState(null);
  const [uploading, setUploading] = useState(false);
  const [transferred, setTransferred] = useState(0);

  const [referenciaPagoMovilPlaza, setReferenciaPagoMovilPlaza] = useState('');
  const [referenciaPagoMovilPlazaFocused, setReferenciaPagoMovilPlazaFocused] = useState(false);
  ...
  const sendingDates = async (referenciaPagoMovilPlaza, image) => {
    // Validaciones
    if (!referenciaPagoMovilPlaza) {
    ...
    try {
      ...
      const pagomovilplazaObj = {
        ...pedido,
        capturapago: captureURL,
        referenciapagomovilplaza: referenciaPagoMovilPlaza,
  ...
  const referenciaPagoMovilPlazaFocus = () => {
    setReferenciaPagoMovilPlazaFocused(true);
  };
    ...
    <SafeAreaView style={styles.container}>
      ...
      <KeyboardAwareScrollView enableOnAndroid>
        <View style={{ alignItems: 'center' }}>
          ...
          <Text style={{ fontWeight: 'bold' }}>Telefono: 04122153376 </Text>
        ...
        <View style={styles.editForm}>
          ...
          <UnderlineTextInput
            ...
            onChangeText={(referenciaPagoMovilPlaza) =>
              setReferenciaPagoMovilPlaza(referenciaPagoMovilPlaza)
            }
            onFocus={referenciaPagoMovilPlazaFocus}
            inputFocused={referenciaPagoMovilPlazaFocused}
          ...
          <View style={{ paddingHorizontal: 60, paddingVertical: 15 }}>
            <Button
              onPress={() => {
                sendingDates(referenciaPagoMovilPlaza, image);
              }}

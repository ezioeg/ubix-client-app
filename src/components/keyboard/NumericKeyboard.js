...
const NumericKeyboard = ({ actionButtonTitle, onPress = () => {} }: Props) => (
  <View style={styles.container}>
    ...
    {actionButtonTitle ? (
      <TouchableOpacity onPress={onPress(actionButtonTitle)} style={styles.keyboardButton}>
        <Text style={styles.actionButtonTitle}>{actionButtonTitle.toUpperCase()}</Text>
      </TouchableOpacity>
    ) : (
    ...
    <TouchableOpacity onPress={onPress('backspace')} style={styles.keyboardButton}>
      <Icon
        name={BACKSPACE_ICON}
        size={24}
        color={Colors.black}
